...
import { GENERAL_NUTRITION_SOURCES } from '../constants/medicalSources';
...
export default function ResultsScreen({ navigation, route }: ResultsScreenProps) {
  ...
  // Sources & methodology modal — visible affordance near the top per Apple 1.4.1.
  ...
  const liveRefs = asArray<ReferenceItem>(result.refs);
  // Apple 1.4.1: the citations section must never be empty — fall back to static
  // authoritative sources when Search Grounding timed out or returned nothing.
  const displayRefs: ReferenceItem[] = liveRefs.length > 0 ? liveRefs : GENERAL_NUTRITION_SOURCES;
  ...
  return (
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      >
        ...
        {/* Sources affordance — citations must be easy to find (Apple 1.4.1) */}
        <TouchableOpacity
          style={styles.sourcesChip}
          onPress={() => { Haptics.selectionAsync(); setSourcesModalVisible(true); }}
          activeOpacity={0.8}
        >
          <Info size={13} weight="bold" color={C.primary} />
          <Text style={styles.sourcesChipText}>Sources & how scores are calculated</Text>
        </TouchableOpacity>
        ...
        {/* Medical disclaimer (Apple 1.4.1) — visible on every tab */}
        <View style={styles.medDisclaimerBox}>
          <Text style={styles.medDisclaimerText}>
            ⚕️ Informational only — not medical advice. Scores and insights are AI-generated estimates.
            Always consult a qualified healthcare provider before making dietary changes based on a health condition.
          </Text>
        </View>
      </ScrollView>

      {/* Sources & methodology modal (Apple 1.4.1) */}
      <Modal visible={sourcesModalVisible} transparent animationType="slide" onRequestClose={() => setSourcesModalVisible(false)}>
        <View style={styles.shareOverlay}>
          <View style={styles.shareSheet}>
            <View style={styles.shareSheetHandle} />
            <View style={styles.shareSheetHeader}>
              <Text style={styles.shareSheetTitle}>Sources & Methodology</Text>
              <TouchableOpacity onPress={() => setSourcesModalVisible(false)} style={styles.shareCloseBtn}>
                <XIcon size={16} weight="bold" color={C.textTertiary} />
              </TouchableOpacity>
            </View>
            <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 440 }}>
              <Text style={styles.sourcesModalSectionTitle}>How scores are calculated</Text>
              <Text style={styles.sourcesModalBody}>
                Vitality, body-system, and organ scores are heuristic 0–10 estimates generated by Google Gemini
                from the nutrient composition of your meal and, when you've added it, your health context.
                They are informational estimates, not clinical measurements. A full breakdown of the six
                physiological systems is in Profile → How we analyze.
              </Text>
              <Text style={styles.sourcesModalSectionTitle}>Citations & sources</Text>
              {displayRefs.map((item, index) => (
                <TouchableOpacity
                  key={index}
                  style={styles.sourcesModalRefRow}
                  onPress={() => item.url && Linking.openURL(item.url)}
                  disabled={!item.url}
                >
                  <Text style={[styles.sourcesModalRefTitle, item.url ? { color: C.primary } : null]}>{item.title}</Text>
                  <Text style={styles.sourcesModalRefDesc}>{item.desc}</Text>
                </TouchableOpacity>
              ))}
              <Text style={styles.sourcesModalDisclaimer}>
                Informational only — not medical advice. Always consult a qualified healthcare provider.
              </Text>
            </ScrollView>
          </View>
        </View>
...
function makeStyles(C: AppColors) {
  return StyleSheet.create({
    ...
    // Sources affordance + modal (Apple 1.4.1)
    sourcesChip: {
      ...
      borderRadius: 14, paddingVertical: 6, paddingHorizontal: 12, marginBottom: 14,
    },
    sourcesChipText: { fontSize: 12, fontWeight: '700', color: C.primary },
    sourcesModalSectionTitle: { fontSize: 13, fontWeight: '800', color: C.textPrimary, marginTop: 14, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.4 },
    sourcesModalBody: { fontSize: 13, color: C.textSecondary, lineHeight: 19 },
    sourcesModalRefRow: { marginBottom: 12 },
    sourcesModalRefTitle: { fontSize: 13, fontWeight: '700', color: C.textPrimary, lineHeight: 19 },
    sourcesModalRefDesc: { fontSize: 12, color: C.textSecondary, marginTop: 2, lineHeight: 17 },
    sourcesModalDisclaimer: { fontSize: 11, color: C.textTertiary, fontStyle: 'italic', lineHeight: 16, marginTop: 6, marginBottom: 16 },
    medDisclaimerBox: {
      backgroundColor: C.bgSecondary, borderRadius: 14, borderWidth: 1, borderColor: C.border,
      padding: 14, marginTop: 18,
    },
    medDisclaimerText: { fontSize: 11, color: C.textTertiary, lineHeight: 17, fontStyle: 'italic' },
    // Alerts tab
    alertRow: {
